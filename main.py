__author__ = "Shane Caldwell (@shncldwll)"
__date__ = "2022"
__version__ = "0.01"
__description__ = """\
burp suite extension to store data http messages in a form that can be tokenized 
...
class BurpExtender(IBurpExtender, ITab, IContextMenuFactory):
    def registerExtenderCallbacks(self, callbacks):
        ...
        self.callbacks.setExtensionName("HTTP_NLP_Collector")
    ...
    def createMenuItems(self, invocation):
        ...
        if itemContext > 0:
            ...
            menuItem = swing.JMenuItem(
                "Send to HTTP NLP Collector", actionPerformed=self.handleHttpTraffic
            )
    ...
    def handleButtonClick(self, event):
        ...
        t = threading.Thread(
            target=self.makeRequest, args=[host, port, protoChoice, request]
        )
        t.daemon = True
        t.start()
    ...
    def makeRequest(self, host, port, protoChoice, request):
        ...
        resp = self.callbacks.makeHttpRequest(
            host, port, protoChoice, request  # string  # int  # bool  # bytes
        )
